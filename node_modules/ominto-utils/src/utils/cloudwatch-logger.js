...
function createCWLog (bunyanLog) {
  // try to parse logs which are already stringified
  if (_.isString(bunyanLog)) {
    try {
      bunyanLog = JSON.parse(bunyanLog);
    } catch (e) {
      bunyanLog = {
        unparseableMessage: bunyanLog,
        parseError: e
      };
    }
  }

  // get time from log (cloudwatch requires separate field from message body)
  var timestamp = bunyanLog.time ? new Date(bunyanLog.time).getTime() : Date.now();

  // create and return cloudwatch-format log body
  var log = {
    message: JSON.stringify(bunyanLog),
    timestamp: timestamp
  };
  return log;
}
