...
const PROGRAMS_ARGS = {
  adPlatformId:       client.siteId, // adPlatformId *required
...
const getMerchants = singleRun(function* () {
  ...
  let programs = yield pagedApiCall('getPrograms', 'result.handler.programs.item', PROGRAMS_ARGS);
  ...
  for( let i=0; i<programIds.length; i++) {
    response = yield apiCall('getProgramDetails', 'result.handler', {programId: programIds[i]});
    response[0].programid = programIds[i];
    programDetails.push(response[0]);
  }

  // get all vouchers available for our siteId
  response = yield pagedApiCall('getVoucherCodes', 'result.handler.voucherCodes.item', VOUCHER_ARGS);
